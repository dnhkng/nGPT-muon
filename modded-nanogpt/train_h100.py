#!/usr/bin/env python3
"""
nGPT trainer for H100 with FineWeb dataset and hyperparameter iteration.
Based on train_mac.py but scaled up for production use.
"""
...
# Parse args
parser = argparse.ArgumentParser(description='nGPT H100 Training')
parser.add_argument('--alpha', type=float, default=0.15, help='Alpha parameter for residual scaling')
parser.add_argument('--logit-scale', type=float, default=10.0, help='Logit scale for cosine similarity')
...
parser.add_argument('--output', type=str, default='results.jsonl', help='Output file for results')
args = parser.parse_args()

device = torch.device('cuda')
print(f"Using device: {device}")
print(f"Hyperparameters: alpha={args.alpha}, logit_scale={args.logit_scale}")
...
def project_weights_to_hypersphere(model):
    """Project all 2D weights to unit hypersphere after optimizer step."""
...
class SimpleBlock(nn.Module):
    def __init__(self, n_embd, n_head, alpha_init=0.15):
        super().__init__()
...
class SimpleGPT(nn.Module):
    def __init__(self, config, alpha_init=0.15, logit_scale_init=10.0):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            SimpleBlock(config['n_embd'], config['n_head'], alpha_init)
...
def load_fineweb_shard(shard_path):
    ...
    with open(shard_path, 'rb') as f:
        # Skip 256-byte header
        f.read(256)
        # Read tokens as uint16
        tokens = np.fromfile(f, dtype=np.uint16)
    return torch.from_numpy(tokens.astype(np.int64))

def load_fineweb_data(data_dir, num_train_shards=5):
...
def main():
    ...
    train_data, val_data = load_fineweb_data(data_dir, num_train_shards=5)
    ...
    # Create model
    model = SimpleGPT(config, alpha_init=args.alpha, logit_scale_init=args.logit_scale).to(device)
