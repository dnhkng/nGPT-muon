...
# Using PyTorch's native attention for ARM64 (GH200) compatibility
# Fallback implementation of flash_attn_varlen_func using PyTorch's SDPA
import torch.nn.functional as F

def flash_attn_varlen_func_fallback(q, k, v, cu_seqlens_q=None, cu_seqlens_k=None, max_seqlen_q=None, max_seqlen_k=None,
                                    causal=True, softmax_scale=None, window_size=(-1, -1)):
    # Convert varlen format to standard format for PyTorch SDPA
    # For now, use standard SDPA with causal masking
    original_dtype = q.dtype
    if softmax_scale is None:
        softmax_scale = 1.0 / (q.shape[-1] ** 0.5)
    # Reshape to [batch, seq, heads, dim] format expected by SDPA
    ...
    attn_output = F.scaled_dot_product_attention(q, k, v, attn_mask=None,
                                                   ...
                                                   scale=softmax_scale)
    # Convert back to original format and preserve dtype
    output = attn_output.transpose(1, 2).squeeze(0).contiguous()
    return output.to(original_dtype) if output.dtype != original_dtype else output

class FlashAttnInterface:
    @staticmethod
    def flash_attn_varlen_func(q, k, v, cu_seqlens_q=None, cu_seqlens_k=None, max_seqlen_q=None, max_seqlen_k=None,
                                causal=True, softmax_scale=None, window_size=(-1, -1)):
        return flash_attn_varlen_func_fallback(q, k, v, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k,
                                                causal, softmax_scale, window_size)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, attn_args: AttnArgs):
        ...
        # q, k = norm(q), norm(k) # QK norm @Grad62304977 - REMOVED for nGPT
        ...
        max_len = args.train_max_seq_len if self.training else min(args.train_max_seq_len, args.val_batch_size // (grad_accum_steps * world_size))
...
class Block(nn.Module):
    def __init__(self, dim: int, head_dim: int, num_heads: int, layer_idx: int):
        ...
        # Optimized: Changed from 0.05 to 0.15 based on Mac testing (best validation loss)
    ...
    def forward(self, x: Tensor, attn_args: AttnArgs):
        ...
        if self.attn is not None:
            attn_out = self.attn(x, attn_args)  # NO norm() wrapper
            x = normalize_ngpt(x + self.alpha_attn * attn_out)
        if self.mlp is not None:
            mlp_out = self.mlp(x)  # NO norm() wrapper
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, head_dim: int, model_dim: int, max_seq_len: int):
        ...
        # Optimized: Changed from ~0.036 (1/sqrt(768)) to 10.0 based on Mac testing
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, seqlens: Tensor, ws_short: int, ws_long: int, mtp_weights: Tensor = None):
        ...
        if self.split_embed:
            ...
            x = normalize_ngpt(F.embedding(input_seq, self.lm_head.weight), dim=-1)
        ...
        # nGPT: x already normalized above, renormalize after smearing
        x = x0 = normalize_ngpt(x[None], dim=-1)
        ...
        x_norm = normalize_ngpt(x, dim=-1)  # Ensure normalized
        w_norm = normalize_ngpt(self.lm_head.weight, dim=-1)  # Normalize weights
        # Ensure dtype consistency for linear operation
        if x_norm.dtype != w_norm.dtype:
            w_norm = w_norm.to(x_norm.dtype)
        logits = F.linear(x_norm, w_norm) * self.logit_scale  # Scaled cosine similarity
        # nGPT: Removed tanh softcapping - cosine similarity is already bounded [-1, 1]
...
adam_labels = ['lm_head', 'value_embed', 'smear_gate', 'x0_lambdas', 'embed', 'alpha', 'logit_scale']
...
def step_optimizers(step: int, optimizers, model, start_transition: bool):
    ...
    if step%2==0:
        ...
        project_weights_to_hypersphere(model)  # nGPT: project weights after Muon step
    # on odd steps, step all params except scalar optimizer during transition
        ...
        for opt in optimizers:
            if in_transition and opt is adam_optimizers[1]:
                ...
                opt.step()
        project_weights_to_hypersphere(model)  # nGPT: project weights after Adam step
...
# Disabled torch.compile for ARM64 compatibility with PyTorch SDPA fallback
# model: nn.Module = torch.compile(model, dynamic=False, fullgraph=True)
# Running in eager mode for initial testing
...
for idx in range(len(ws_schedule)):
    ...
    for step in range(warmup_steps):
        ...
        if step % 2 == 0:
            for opt in muon_optimizers: opt.step(), opt.zero_grad(set_to_none=True)
            project_weights_to_hypersphere(model)  # nGPT: project weights
        else:
